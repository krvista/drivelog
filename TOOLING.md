...
## 알려진 문제: prune 의 정렬 기준 (미해결)

`prune` 은 route 이름을 문자열로 정렬해 "오래된 것"을 판단한다. 그런데 기기를 다시
빌드하거나 초기화하면 route 카운터가 `00000000` 부터 다시 시작한다.
2026-09-07 이 상황이 실제로 발생했다. 브랜치에는 `00000061` 까지 올라가 있는데
기기의 새 데이터는 `00000005` 다. 이대로 `prune` 을 돌리면 **가장 최신 주행이
가장 오래된 것으로 취급돼 먼저 지워진다.**

날짜로 바꾸는 것도 답이 아니다. 주차장에서만 움직이면 GPS 를 못 잡아서
기기 시각이 실제와 어긋난다.

그래서 `prune` 은 고치기 전까지 쓰지 말 것. 기본값이 "계획만 출력" 이고
실제 삭제에는 `--yes` 가 필요하므로 실수로 지워지지는 않는다.
