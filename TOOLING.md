...
# 이 PC 의 도구·문서를 브랜치 사본으로 맞춘다 (drivelog.conf 는 건드리지 않는다)
./drivelog.sh sync --profile ccnc
```

## 도구 사본만은 낡을 수 있다

데이터의 상태는 원격만 보면 되지만, 도구 자체는 각 PC 에 복사본으로 존재한다.
그래서 **도구만은 PC 마다 낡을 수 있고, 낡은 사본은 조용히 기능을 빠뜨린다.**

2026-09-08 실제로 그랬다. 회사 PC 의 사본이 `order.txt` 를 모르던 판이라
route `00000007` 을 40개 파일째 올리면서 원장을 갱신하지 않았다. 그대로 뒀으면
`prune` 이 그 route 를 "원장에 없음 = 가장 오래됨" 으로 보고 가장 먼저 지웠을 것이다.
(`init-order` 를 다시 돌려 히스토리에서 복원했다.)

그래서 모든 명령이 시작할 때 이 PC 의 `drivelog.sh` 가 브랜치의 것과 같은지 비교하고
다르면 경고한다. `sync` 가 브랜치 사본으로 맞춰준다.
**다른 PC 에서 작업하다 왔으면 무엇을 하기 전에 `sync` 부터 돌릴 것.**
